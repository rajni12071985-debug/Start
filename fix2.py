...
# We need to replace from `function generatePost()` up to `function showLandingPage`
start_idx = text.find('function generatePost() {')
end_idx = text.find('function showLandingPage(p, salePrice, offerText) {')

if start_idx != -1 and end_idx != -1:
    # Back up the comment before showLandingPage
    # let's find the `/* ─── 9.`
    end_comment = text.rfind('/* ', start_idx, end_idx)
    end_idx = text.find('/* ', end_comment) # which is end_comment
    
    clean_code = """function generatePost() {
...
"""
    ...
    new_text = text[:start_idx] + clean_code + text[end_idx:]
